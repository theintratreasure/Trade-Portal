...
import { useSignup, useVerifySignupOtp } from "@/hooks/useAuth";
...
export default function SignupPage() {
    ...
    const handleSignup = () => {
        ...
        signup.mutate(
            ...
            {
                onSuccess: () => {
    setToast("Verification email sent. Please check your email.");
    setStep("verify"); // 👈 ye zaroori hai
},
            }
        );
    };
    ...
    return (
        <div className="relative min-h-screen flex items-center justify-center bg-[var(--bg-main)] px-4">
            ...
            <div className="relative w-full max-w-md rounded-2xl border border-[var(--border-glass)] bg-[var(--bg-card)] backdrop-blur-xl shadow-2xl p-6 space-y-6">
              ...
              "
                        >
                            {signup.isPending ? "Sending OTP..." : "Create account"}
                        </button>
                    </>
                )}
